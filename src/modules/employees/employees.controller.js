...
export const getAccouentForBusSuperviosr=async (req,res,next)=>
{
try
{
// check first if this busSuperviosr have an bus:
const {_id}=req.data;
const busSup=await busModel.findOne({busSupervisor:_id}).populate([{path:"children"}]);
if(!busSup)
    {
        return next(new Error("the busSUperviosr not has an bus yet"));
    }
    // check on the account id:
    const {accountId}=req.query;
    if(accountId)
        {
            const childData=await childrenModel.findOne({_id:accountId,bus:busSup._id});
            if(!childData)
                {
                    return next(new Error("this child is not asssgin to your bus or the id is incorrect"));
                }
            return res.json({success:true,data:childData});
        }
    const {children}=busSup;
    // return the response:
...
// get for the supserviosr all the accouent data or spAccount:
export const getAccountsForSuperviosr=async (req,res,next)=>
{
try
{
const {_id}=req.data;
// check first if the supeervisor have an gorup:
const supGroup=await groupModel.findOne({groupSupervisor:_id}).populate([{path:"childrenGroup",populate:[{path:"bus"},{path:"groupId"}]}]);
if(!supGroup)
    {
        return next(new Error("the supervisor not have a group yet to get the children Accounts"));
    }
   const {accountId} =req.query;
   if(accountId)
    {
        const childDate=await childrenModel.findOne({_id:accountId,groupId:supGroup._id});
        if(!childDate)
            {
                return next(new Error("the child is not exists in your group or the id is incorrect"));
            }
        return res.json({success:true,data:childDate})    
    }
    const {childrenGroup}=supGroup;
    // returnt the response:
    return res.json({success:true,data:childrenGroup});
}
catch(err)
{
    return next(err);
