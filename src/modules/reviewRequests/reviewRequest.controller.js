...
export const getWaitingLists=async (req,res,next)=>
{
try
{
const {_id}=req.data;
// get all the requests in the waiting lists:
const allWaitingLists=await requestsModel.find({state:'waiting'}).sort("updatedAt createdAt");
// returnt he resposne:
return res.json({success:true,lists:allWaitingLists});
}
catch(err)
{
    return next(err);
...
// chane the state of waiting lists:
export const changeTheSteteOfWaiting=async (req,res,next)=>
{
try
{
...
if(!request)
    {
        return next(new Error("there is no request by this id it can may be deleted or other"));
    }
    if(request.state!="waiting"&&request.state!="refused"&&request.state!="underRevising")
        {
            return next(new Error(`orry this request you have not authorized to see it because it is in the ${request.state}`));
        }
    if(state)
        {
            if(state=="interviewing")
                {
                   request.state="interviewing";
                   request.evaluatedBy="_id";
                   await request.save();
                   const sendingEmailone=await sendingEmail({to:request.email,subject:"the new result of your waitingRequets",text:"this is the result of your waiting request",html:`<div style='text-align:centers;'><h2>the results</h2><h1>congratulations the request is initially accepted,your requests data is actullay correct and apply the condition and we will <br/><span style='text-decoration:underline;background-color:red;color:black;'>you will go to the interviewing step and we will send you an email by the interviewing time</span></h1></div>`});
                   if(!sendingEmailone)
                    {
                        return next(new Error("the email is not sedn the connection may be unstable or the email is not valid"));
                    }
                }
            else if(state=="refused")  
                {
                   if(!condition)
                    {
                        return next(new Error("you must send the condition with refused request"));
                    }      
                    request.state='refused';
                    request.evaluatedBy=_id;
                    request.condition=condition;
                    await request.save();
                    const sendingEmailone=await sendingEmail({to:request.email,subject:"the new result of your waitingRequets",text:"this the result of the your waitingRequet",html:`<div style='text-align:center;'><div><h2>the results is:</h2><h1>sorry your requests is refused</h1></div><div><h2>the condition of refused:</h2><h1>${condition}</h1></div></div>`});
                    if(!sendingEmailone)
                        {
                            return next(new Error("the email is not sedn the connection may be unstable or the email is not valid"));
                        }
            ...
            // send the response:
            return res.json({success:true,messsage:`the request state is updated sucessfully to ${state}`});    
        }    
}
catch(err)
{
