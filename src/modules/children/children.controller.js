...
// get the employee contact fro the children:
export const getContact=async (req,res,next)=>
{
try
{
    // get  the id og th echild:
    const {_id}=req.data;
    // check ont he child first:;
    const child=await childrenModel.findOne({_id:_id}).populate([{path:"bus",populate:[{path:"busSupervisor"}]},{path:"groupId",populate:[{path:"groupSupervisor"}]}]);
    if(!child)
        {
            return next(new Error("there is no child by this id or the accouent may be deleted"));
        }
    // get the busSuperviosr first:
    const {bus,groupId}=child;
    // return the response:
    return res.json({sucess:true,supervisor:groupId,busSupervisor:bus});
}
catch(err)
{
    return next(err);
